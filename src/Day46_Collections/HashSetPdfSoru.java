...
public class HashSetPdfSoru {
	...
	public static void main(String[] args) {
		...
		for (int i = 1; i <= 100; i++) { // 100 tane random sayý getirdim,ekledim
			ts.add(r.nextInt());
		}
		Long bitisTree = System.currentTimeMillis(); // iþlemi bitirdiði süre

		Long baslangicHash = System.currentTimeMillis(); // iþleme baþlama süresi
		for (int i = 1; i <= 100; i++) {
			hs.add(r.nextInt());
		}
		Long bitisHash = System.currentTimeMillis(); // iþlemi bitirdiði süre
...
Set<Integer> hs2=new HashSet<>();   // yeni bir Hashset ve TreeSet oluþturdum.
...
Long bitis3=System.currentTimeMillis();  // iþlemi bitirdiði süre
...
// Her zaman ayný olmuyor,degiþiyor.
